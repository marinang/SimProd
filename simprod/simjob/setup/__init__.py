...
def checksiminputs(job):
        ...
        with warnings.catch_warnings():
            ...
            if job.stripping == None:
                ...
                if len(args) > 1:
                    warnings.warn(
                        red(
                            "Default stripping version {0} used. {1} versions are available.".format(
                                job.stripping, args
                            )
                        ),
                        stacklevel=2,
                ...
                raise NotImplementedError(
                    "Stripping version {0} is not available for {1} {2}! Only {3}!".format(
                        job.stripping, job.year, job.simcond, args
                    )
                )

    if job.simcond == "Sim09b" and job.year in [2011, 2017, 2018]:
        raise NotImplementedError(
            "{0} setup is not (yet) implemented for {1}!".format(job.year, job.simcond)
        )
    ...
    elif job.simmodel == "BcVegPy" and job.simcond not in [
        "Sim09e",
        "Sim09g",
        "Sim09h",
    ]:
        raise NotImplementedError(
            "BcVegPy is not implemented for {0}!".format(job.simcond)
        )

    if job.redecay and job.simcond == "Sim09b":
        raise NotImplementedError(
            "ReDecay is not implemented for {0}!".format(job.simcond)
        )
